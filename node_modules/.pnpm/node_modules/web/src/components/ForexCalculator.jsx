...
export const ForexCalculator = ({ onCalculate }) => {
  ...
  // Format values based on currency pair
  const getDecimalPlaces = (pair) => {
  ...
  useEffect(() => {
    // Update exit point when entry and TP change
    if (formData.entryPoint && formData.tpPips) {
      const pipSize = formData.pair === "XAUUSD" ? 0.1 : 
                      formData.pair.includes("JPY") ? 0.01 : 0.0001;
      
      const newExitPoint = formData.direction === 'buy'
  ...
  const handleCalculate = (e) => {
    ...
    // Simulate API delay
    setTimeout(() => {
      const { pair, lotSize, entryPoint, exitPoint, direction, tpPips, slPips, accountCurrency } = formData;
